  ...
  template: `
    <ion-content style="--background: none; top: 20%">
      <div style="background: #1e1e1e; padding: 2%; margin-left: 10%; margin-right: 10%; border: 1px solid grey">
        ...
        <ion-item style="">
          <u (click)="createNewThread()" style="cursor: pointer; color: white; white-space: nowrap">Opret en tråd</u>
          <ion-searchbar style="" [debounce]="1000" [formControl]="searchterm" (ionInput)="searchOnThreads()"> </ion-searchbar>
        </ion-item>
        </div>
...
export class TopicComponent {
  ...
  async searchOnThreads() {
    this.route.params.subscribe(async (params) => {
      ...
      if (searchTermLower) {
        const call = this.http.get<Thread[]>(`${environment.baseUrl}/searchOnThreads?searchTerm=${searchTermLower}&topicId=${topicId}`);
